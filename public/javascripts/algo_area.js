...
class AlgoArea {
    ...
    _setLineNumbers() {
        ...
        const digits = _numOfDigits(this._numOfOperations > 0 ?
        ...
        if(this._hlManager.highlightedLines > 0) {
            ...
            */
            ...
            for(let i = 0; i < lastLineWithHighlighting; i++) {
                //non-highlighted lines may change, because they are no operations
                if((i < this._hlManager.offset || this._hlManager.isHighlighted(i)) //the header and highlighted lines are not allowed to change (but comments are)
